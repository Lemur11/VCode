...
Intake::Intake(double length, double topPos, double sensorPos) : fullLength(length), top(topPos), sensorPos(sensorPos) {
...
void Intake::update() {
  ...
  if (intakeD.get() < 70) {
    if ((sortState == RED && intakeC.get_hue() > 120) or (sortState == BLUE && intakeC.get_hue() < 40)) {
